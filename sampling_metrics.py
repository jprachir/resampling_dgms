...
os.environ['TF_CPP_MIN_LOG_LEVEL'] = '3' # avoids polluting standard errors with info, warnings, errors, gpu's memory allocation log while printing
...
def get_activation_stats(images, 
    ...
    print(logits.shape, features.shape) #(9900,1008)(9900,2048)
    ...
    for idx in range(1): #30GB
        ...
        with torch.no_grad():
            for _, gen_data in enumerate(gen_test_loader):
                print(_, end=' ', flush=True) # 0 1 2 3 ...
                xte_gen.append(gen_data.numpy())
                gen_data = gen_data.to(device)
                logits = model(gen_data) # crashing here on gpu oom, test_b_s=300, tf=0.14.0

...
def get_real_data(real_datadir):
    ...
    xtrva_real = datasets.CIFAR10(root=real_datadir, download=False)
    ...
    xte_real = torch.from_numpy(datasets.CIFAR10(root=real_datadir, download=False, train=False).data.transpose((0, 3, 1, 2)))
...
def get_gen_data(gen_datadir):
    ...
    xtr_gen, xva_gen, xte_gen = torch.from_numpy(xgen['train_data'].transpose((0,3,1,2))),\
                                torch.from_numpy(xgen['valid_data'].transpose((0,3,1,2))), \
                                torch.from_numpy(xgen['test_data'].transpose((0,3,1,2)))
...
def get_loaders(
    ...
    xtr_real, xva_real, xte_real = get_real_data(real_datadir) #array-> tensors

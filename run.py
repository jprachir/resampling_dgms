...
def test_calibration(args, model, device, data_loader, n_bins=10, savefile=None):
    ...
    # print("done with extended calibration curve")
    calibration_error = mean_squared_error(fraction_of_positives, mean_predicted_value, sample_weight=bin_weights)
    print('Calibration error', calibration_error) #Calibration error 1.6956276324930258e-05

    if savefile is not None:
        cal_savefile = os.path.join(args.sampledir, 'calib_'+savefile[:-4]) # savefile[:-3] calib_plotting
...
def main():
    ...
    parser.add_argument('--sampledir', type=str, default='./samples/vae')
    parser.add_argument('--modeldir', type=str, default='./models/vae')
    ...
    parser.add_argument('--use-feature', dest='use_feature', action='store_true', default=True,
    help='trains a model on inception v3 features if true')
    parser.add_argument('--use-mlp', dest='use_mlp', action='store_true', default=True,
    ...
    parser.add_argument('--flatten', dest='flatten', action='store_true', default=True,
    help='flatten the weights to power alpha if True')
    parser.add_argument('--clip', dest='clip', action='store_true', default=True,
    ...
    if args.train:
        ...
        for epoch in range(1, args.epochs + 1):
            ...
            print("finished training model in {} seconds for epoch {}".format(end-start, epoch))
            _, valid_acc = test(args, model, device, valid_loader)
            # saving best model in 10 epochs
    ...
    # if args.calibration: # TypeError: label_binarize() takes 1 positional argument but 2 were given
    #     # test_calibration(args, model, device, test_loader, n_bins=10, savefile=None)
    #     test_calibration(args, model, device, test_loader, n_bins=10, savefile="plotting.png")


    # if args.bias_variance: # ip: real_features, gen_features from tf inception, returns mu1,mu2, no- train, test, compute_iw_scores
    #     compute_bias_variance(args, model, device, kwargs)

    # if args.reference: #working: no train, no test required,
    #     compute_train_test_scores(args, model, device, kwargs)

    compute_iw_scores(args, model, device, kwargs) # working: no train, test required
